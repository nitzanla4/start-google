...
import java.util.*;
    ...
    public class FootballTeam {
        public enum TeamsNames {BARCELONA, REALMADRID,RISHON, HAIFA;}
        static  final int Team_size=11;
        static private  TeamsNames name;
        private static Set players;

        public TeamsNames getName() {return name;}

        public  Set getPlayers() { return players; }

        public static void setPlayers(Set players) {FootballTeam.players = players; }

        public FootballTeam(TeamsNames name) { //name of the team, positions of the players
            // need to check if name is from names lists!!!!
            this.name=name;
            players=new HashSet();}


        public static FootballTeam createNewTeamByPositions(TeamsNames name, int[] pos)
        {
        ...
        public static List<String> createDicList(TeamsNames teamName)
        {
            if (Validation.equalsToBarcelona(teamName)) // both option-upper and lower
            {
                NameGenerator myDic = NameGenerator.createWantedDic("dicBarcelona");
                return myDic.getNamesSet();
            }
            else if (Validation.equalsToRishon(teamName))
            {
        ...
        public static Set <Player> create_team(TeamsNames teamName, int[] pos)
        {
            ...
            {
                int jersy_shirt=Utils.RandomNumber.getRandomNumber(); //get a random shirt number (unique number)
                while (teamMap.containsKey(jersy_shirt)) //we already have this jersey number
                { jersy_shirt=Utils.RandomNumber.getRandomNumber();}
                String name=Generate.getRandomName(teamName);
                teamMap.put(jersy_shirt, name); //for future check (name and jersy number)
                String position=getPosition(myList);

                team.add(createPlayerUsingFactoryMetByPosition(name,jersy_shirt,Generate.getRandomGrade(),position));
            }
            return team;
        }
